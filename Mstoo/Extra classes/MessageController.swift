...
//        observeMessages()
    ...
    func fetchUserAndSetNavBarTitle() {
        ...
        Database.database().reference().child("users").child(uid).observe(.value, with: { (snapshot) in
            ...
            if var userInfoDict = snapshot.value as? [String : String] {
                let user = Person()
                userInfoDict["email"] = "Real.minkle@gmail.com" as String
                userInfoDict["name"] = "Minklereal" as String
//                userInfoDict["profileImageUrl"] = "https://firebasestorage.googleapis.com/v0/b/mstoo-192611.appspot.com/o/Profile_images%2Fmy_profile_image_AvQuFc2YqZb3zV4DNWBgLbEwPVl1.png?alt=media&token=242d67de-9e75-41e4-b102-a7c64775b157" as String
...
//        nameLabel.widthAnchor.constraint(equalToConstant: 40).isActive = true
    ...
    func showChatControllerForUser(user: Person) {
        let topctlr = topMostController()
        let chatLogController = ChatLogController(collectionViewLayout: UICollectionViewFlowLayout())
        chatLogController.user = user
        topctlr.present(chatLogController, animated: true, completion: nil)
